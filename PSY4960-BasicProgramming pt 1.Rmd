...
```{r}
favoriteCoffee<- "caramel highrise"
temperature<- 50
...
```{r}
animals<- c("dragon", "elephant", "dog", "cat", "toad", "lemur")
```
2)Make a vector of the number of cookies you think each animal could eat
```{r}
cookiesEaten<- c(450,200, 10,5, .2, 8) 
```
...
```{r}
animalDat<- cbind.data.frame(animals, cookiesEaten) 
animalDatTwo<-rbind.data.frame(animals, cookiesEaten)
animalDAtThree<- data.frame(animals, cookiesEaten)
```
...
```{r}
tvShows<-c("welcome to wrexham", "criminal minds", "ozark", "what we do in the shadows", "the office")
ratings<- c(9,8,10, 10, 2)
```

3.  Make this variable into a data frame.

```{r}
tvDat<-cbind.data.frame(tvShows, ratings)
```
...
```{r}
ChickOne_ChickWeight<- subset(ChickWeight, ChickWeight$Chick==1)
summary(ChickOne_ChickWeight)
...
"ChickWeight %>% filter(Chick==1) %>% select(weight)" #Dr. W's R didn't like this. 

C1<-data.frame(ChickWeight %>% filter(Chick==1))
range(C1 %>%  select(weight))

range(data.frame(ChickWeight) %>% filter(Chick==1) %>% select(weight))
```
